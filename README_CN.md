...
|------|------|-----|
| /start | 显示帮助菜单 | - |
| /bind [symbol] [interval] [EMA] | 绑定已有挂单到追踪 | /bind BTC 4h 21 |
| /list | 查看所有追踪订单 | - |
| /remove [ID] | 删除追踪订单 | /remove BTCUSDT_4h_EMA21_BUY |
| /ema [symbol] [interval] | 查询EMA值 | /ema ETH 1h |
| /price [symbol] | 查询当前价格 | /price BTC |
| /balance | 查询账户余额 | - |
| /status | 机器人运行状态 | - |
| /start_bot | 启动追踪 | - |
| /stop_bot | 停止追踪 | - |
...
| 类型 | 支持值 | 
|------|-------|
| EMA 周期 | 21 / 55 / 100 / 200 |
| 时间周期 | 15m / 1h / 4h / 1d / 1w / 1M |
| 更新阈值 | 0.3% (可在代码中修改 price_threshold) |
...
| quantity | 数量 | 
| leverage | 杠杆倍数 |
| margin_type | 保证金模式 (CROSSED/ISOLATED) |
...
### ⚠️ 风险事项
1. 合约交易具有高风险，可能导致本金全部损失
2. 请使用小资金测试，确认策略有效后再投入
3. 建议先在测试网验证
4. 请妥善保管 API Key，不要泄露给他人
5. API 权限仅开启「合约交易」，不要开启「提现」

## 🐛 常见问题
1. 确保使用最新代码，calculate_ema 方法已排除未完成的K线。
2. 服务器时间不同步，代码已自动处理，如仍有问题请检查系统时间。
3. 确保绑定时正确设置 position_side（LONG/SHORT），代码会自动检测持仓模式。
